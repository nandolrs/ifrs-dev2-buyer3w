...
function AtividadesObter()
{
    ...
    if(outVideo != null)
    {
      let perc = outVideo.currentTime / (outVideo.duration==null || outVideo.duration==0 ? 1 : outVideo.duration) ; 

      if(!outVideo.paused || (outVideo.paused && perc==1))
        {

            if(outVideo.currentTime == 0)
            {
              return;
            }
            ...
            let inscricaoAtividade = new InscricaoAtividade(
                 ...
                 ,outVideo.currentTime // posicao
                 ,outVideo.duration // tamanho
            );
